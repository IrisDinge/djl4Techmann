...
# Learning parameters
checkpoint = None
    #r'C:\Users\TME-DJ\MobileNetV3-SSD-Compact-Version\checkpoint_ssd300.pth.tar' #  path to model checkpoint, None if none
...
def main():
    ...
    criterion = MultiBoxLoss(priors_cxcy=model.priors_cxcy.to(device)).to(device)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        """
        # Torchscript
        for i, (image, boxes, labels, _) in enumerate(train_loader):
            images = image
        traced_script_module = torch.jit.trace(model, images)
        sm = torch.jit.script(model(images))
        traced_script_module.save("./traced_custom_model.pt")
        """
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (images, boxes, labels, _) in enumerate(train_loader):
        ...
        # Loss
        loss = criterion(predicted_locs, predicted_scores, boxes, labels).to(device) # scalar
